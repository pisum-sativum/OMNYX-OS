...
const MOCK_REPORTS: Record<string, AIAnalysisResult> = {
  microphone_access: {
    ...
    behavioralObservations: [
      'Audio sensor activated during idle window with no user-initiated trigger detected.',
      'Access duration exceeded typical voice-assistant activation by 4x.',
      'No concurrent media application active to justify microphone use.',
    ],
    ...
    agentInsights: [
      ...
      { agent: 'ORACLE', insight: 'Pattern probability 87% match to passive audio collection behavioral signature.' },
      { agent: 'PHANTOM', insight: 'Covert activation window aligns with known harvesting interval patterns.' },
    ],
    threatCategory: 'surveillance',
  },
  camera_access: {
    summary: 'Visual sensor accessed outside expected application context.',
    explanation: 'Camera hardware was activated without corresponding UI activity. This access pattern does not align with the application declared functionality. Visual data capture in background contexts may indicate covert image or video collection.',
    ...
    behavioralObservations: [
      'Camera hardware enabled without foreground UI context or user gesture.',
      'Capture duration inconsistent with legitimate photo/video functionality.',
      'No media preview surface rendered during activation window.',
    ],
    ...
    agentInsights: [
      ...
      { agent: 'ANALYST', insight: 'Access pattern deviates from declared application function by 3 standard deviations.' },
      { agent: 'PHANTOM', insight: 'Stealth capture signature - no viewfinder surface rendered during activation.' },
    ],
    threatCategory: 'surveillance',
  },
  clipboard_read: {
    ...
    behavioralObservations: [
      'Clipboard read event occurred 2.3 seconds after device wake with no paste gesture recorded.',
      'Network request followed clipboard access within 800ms.',
      'Access frequency inconsistent with declared application functionality.',
    ],
    ...
    agentInsights: [
      ...
      { agent: 'SCOUT', insight: 'Network request followed clipboard access within 800ms - exfiltration timing signature.' },
      { agent: 'ORACLE', insight: 'Confidence 94%: this access pattern precedes data transmission in 91% of observed cases.' },
    ],
  ...
  location_access: {
    ...
    behavioralObservations: [
      'Location polling interval: every 45 seconds in background state.',
      'No geofencing or navigation context active during polling window.',
      'Coordinate precision exceeds what utility functions require.',
    ],
    recommendedAction: 'Restrict location permission to "only while using" and disable background app refresh.',
    confidence: 0.89,
    agentInsights: [
      { agent: 'SCOUT', insight: 'Location polling interval every 45 seconds in background state - tracking cadence.' },
      { agent: 'SENTINEL', insight: 'No geofencing or navigation context active to justify background access.' },
      { agent: 'ORACLE', insight: 'Movement profile construction viable at this polling rate within 6 hours.' },
    ],
    threatCategory: 'tracker',
  },
  network_request: {
    summary: 'Anomalous network telemetry detected to unregistered endpoint.',
    explanation: 'Outbound network traffic was observed to an endpoint not associated with the application declared services. The request payload size and timing suggest data aggregation. Traffic analysis indicates potential data transmission to a third-party collection infrastructure.',
    severityReason: 'Network requests to undisclosed endpoints fall outside the application stated data practices and may violate user consent.',
    behavioralObservations: [
      'Destination endpoint not in known CDN or application server registry.',
      'Request frequency increased 3x during low-use periods.',
      'Payload size consistent with compressed device telemetry bundle.',
    ],
    recommendedAction: 'Block application network access via firewall rule and monitor for repeated connection attempts.',
    confidence: 0.78,
    agentInsights: [
      { agent: 'SCOUT', insight: 'Destination endpoint not in known CDN or application server registry.' },
      { agent: 'ANALYST', insight: 'Request frequency increased during low-use periods - reverse correlation with user activity.' },
      { agent: 'PHANTOM', insight: 'Traffic signature consistent with covert telemetry pipeline - low visibility, high cadence.' },
    ],
  ...
  tracker_detected: {
    ...
    behavioralObservations: [
      'SDK signature matches OMNYX tracker database entry at high confidence.',
      'Tracker operates across 340+ applications in current detection pool.',
      'Data transmission cadence consistent with commercial analytics pipeline.',
    ],
    ...
    confidence: 0.96,
    agentInsights: [
      { agent: 'ANALYST', insight: 'SDK signature matches OMNYX tracker database at 96% confidence.' },
      { agent: 'SENTINEL', insight: 'Tracker actively reading device identifiers outside declared scope.' },
      { agent: 'ORACLE', insight: 'Cross-app identity correlation likely complete within 72 hours of continued operation.' },
      { agent: 'SCOUT', insight: 'Data transmission cadence consistent with commercial analytics pipeline upstream.' },
    ],
    threatCategory: 'tracker',
  },
  suspicious_permission: {
    ...
    behavioralObservations: [
      'Permission set scores 8.2 out of 10 on anomaly index for application category.',
      'Three permissions flagged as atypical for stated application function.',
      'Combination of location plus microphone plus contacts matches data broker collection profile.',
    ],
    recommendedAction: 'Audit each requested permission individually and deny any not essential to core functionality.',
    confidence: 0.83,
    agentInsights: [
      { agent: 'ANALYST', insight: 'Permission set scores 8.2 on anomaly index for this application category.' },
      { agent: 'SENTINEL', insight: 'Three permissions flagged as atypical for stated app function.' },
      { agent: 'ORACLE', insight: 'This permission profile precedes aggressive data collection in 79% of matched cases.' },
    ],
  ...
  background_activity: {
    ...
    behavioralObservations: [
      'CPU utilization spike: 34% during screen-off state, 8x normal idle baseline.',
      'Memory allocation pattern consistent with file indexing or data compression.',
      'Process window timed to coincide with device charging and Wi-Fi connection.',
    ],
    recommendedAction: 'Enable background app restriction for this application and review battery usage statistics.',
    confidence: 0.81,
    agentInsights: [
      { agent: 'SENTINEL', insight: 'CPU utilization spike: 34% during screen-off - 8x normal idle baseline.' },
      { agent: 'ANALYST', insight: 'Memory allocation pattern consistent with file indexing or data compression.' },
      { agent: 'PHANTOM', insight: 'Process window timed to device charging and Wi-Fi - transmission preparation signature.' },
    ],
